...
| 数字格式、日期格式 | ✅ 存活 | |
| 列宽 | ✅ 存活 | |
| 图表、图片 | ✅ 往返实测存活（代理证据） | 见下方「图表/图片实测补记」 |
...
## 图表/图片实测补记（2026-08-30 实现日）

原计划用 xlwings + 真 Excel 造带图表的 .xls 样本，但新版 Mac Excel 的 AppleScript
已存不了 Excel98to2004 (.xls) 格式——`save_workbook_as` 一律 OSERROR -50
「Parameter error」，换系统临时目录路径也复现，确认是格式本身被砍、不是沙箱路径问题。

改用 LO 往返实测：openpyxl 造带柱状图 + 嵌入 PNG + `=SUM` 公式的 .xlsx →
soffice 转 .xls → soffice 再转回 .xlsx（第二步与生产命令完全一致）。结果三项全存活：
终产物里有 `xl/charts/chart1.xml`、`xl/media/image1.png`，公式仍是 `=SUM(B2:B4)`。

证据强度说明：样本的 BIFF 是 LibreOffice 亲笔而非 Excel 亲笔，验证的是 LO 的 BIFF
图表读写往返——作为「LO 能从 .xls 里读出图表」的代理证据可用，但 Excel 写出的 BIFF
图表结构更杂，个别真实文件仍可能有出入。已发布的弹窗文案「图表、图片可能有出入」
保持不变，正好覆盖这个残余不确定性。

## 遗留未核实点

1. ~~图表/图片保真度需实测~~ → 已实测（LO 往返，见上），残余不确定性仅剩
   Excel 亲笔 BIFF 的个别结构差异，文案已对冲。
