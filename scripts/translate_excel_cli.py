#!/usr/bin/env python3
"""
Command-line entrypoint for the Excel translator.
"""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the local Excel translator without opening the Streamlit UI."
        ),
    )
    parser.add_argument(
        "source",
        help="Excel file path or folder path to translate.",
    )
    parser.add_argument(
        "--target-lang",
        help="Target language code or display name, such as en or 英文.",
    )
    parser.add_argument(
        "--source-lang",
        help="Source language code or display name, such as zh or 中文.",
    )
    parser.add_argument(
        "--output-dir",
        help="Optional custom output root directory.",
    )
    parser.add_argument(
        "--allow-xls-fallback",
        action="store_true",
        help="Allow pure-code fallback when legacy .xls needs conversion.",
    )
    parser.add_argument(
        "--engine-mode",
        choices=("cloud", "local"),
        help="Override the saved engine mode for this run only.",
    )
    parser.add_argument(
        "--cloud-provider",
        help="Override the saved cloud provider for this run only.",
    )
    parser.add_argument(
        "--cloud-model",
        help="Override the saved cloud model for this run only.",
    )
    parser.add_argument(
        "--cloud-base-url",
        help="Override the saved cloud base URL for this run only.",
    )
    parser.add_argument(
        "--ollama-model",
        help="Override the saved Ollama model for this run only.",
    )
    parser.add_argument(
        "--concurrency",
        type=int,
        help="Override the saved cloud concurrency for this run only.",
    )
    parser.add_argument(
        "--ollama-concurrency",
        type=int,
        help="Override the saved Ollama concurrency for this run only.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        help="Override the saved batch size for this run only.",
    )
    parser.add_argument(
        "--domain-preset",
        help="Override the saved domain preset for this run only.",
    )
    parser.add_argument(
        "--custom-prompt",
        help="Set a one-off custom prompt and force domain preset to 自定义.",
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    parser.add_argument(
        "--quiet",
        action="store_true",
        help="Suppress progress output and only print the final result.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = run_translation_path(
            args.source,
            settings=settings,
            allow_xls_fallback=args.allow_xls_fallback,
