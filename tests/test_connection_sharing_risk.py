...
def test_two_tasks_on_different_connections_do_not_form_a_shared_group():
...
def test_two_tasks_without_spreading_share_one_group_and_are_flagged():
...
def test_each_pool_entry_has_its_own_upstream_identity():
    app = _two_vendor_settings(spread=True)
    with patch("core.model_roles.get_key", side_effect=_fake_key):
        signatures = {
            api_group_signature_from_config(
